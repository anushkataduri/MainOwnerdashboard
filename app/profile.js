...
export default function OwnerProfile() {
    ...
    structure: [
      {
        floor: "1st Floor", rooms: [
          { id: "101", beds: 4, occupied: 3, tenants: ["Rahul", "Amit", "Suresh"] },
          { id: "102", beds: 4, occupied: 4, tenants: ["Vicky", "Sonu", "Kiran", "Deepak"] }
        ]
      },
      {
  ...
  const sortedCategories = Object.keys(groupedExpenses)
    .map(name => ({ name, value: groupedExpenses[name] }))
    .sort((a, b) => b.value - a.value);

  const topCategories = sortedCategories.slice(0, 3);
  const otherValue = sortedCategories.slice(3).reduce((s, c) => s + c.value, 0);

  const chartData = [
    { name: 'Net Profit', value: Math.max(0, netProfit), color: COLORS.SUCCESS || '#4CAF50' },
    ...topCategories.map((c, i) => ({
      name: c.name,
      value: c.value,
      color: [COLORS.ERROR, '#2196F3', '#FF9800', '#9C27B0'][i] || COLORS.PRIMARY_LIGHT
    })),
  ];
  if (otherValue > 0) chartData.push({ name: 'Others', value: otherValue, color: '#9E9E9E' });

  const totalForChart = totalIncome || 1;
  let cumulativeValue = 0;
  const pieSegments = chartData.map(item => {
    const percentage = (item.value / totalForChart) * 100;
  ...
  return (
    <View style={{ flex: 1 }}>
        ...
        <View style={styles.content}>
            ...
            <View style={styles.chartContainerHorizontal}>
              ...
              <View style={styles.pieContainer}>
                ...
                <View style={styles.pieInner}>
                  ...
                  <Text style={styles.pieCenterLabel}>PROFIT</Text>
                </View>
              </View>
              ...
              <View style={styles.statsBox}>
                ...
                <View style={[styles.statsRowHorizontal, { borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.2)', paddingTop: 5, marginTop: 5 }]}>
                  <Text style={[styles.statsLabel, { fontWeight: '800' }]}>Net Profit</Text>
                  <Text style={[styles.statsValueSmall, { color: COLORS.SUCCESS || '#4CAF50', fontSize: 18 }]}>₹{netProfit}</Text>
                </View>
              </View>
            </View>
          ...
          <View style={styles.expenseBox}>
            ...
            <TextInput
              ...
              multiline
            />
            <TouchableOpacity style={styles.addButton} onPress={addExpenseItem}>
              <Text style={styles.addButtonText}>Add Expense</Text>
            </TouchableOpacity>

            <TouchableOpacity
              style={[styles.addButton, { backgroundColor: COLORS.BLUE_LIGHT, marginTop: 10 }]}
          ...
          {showExpenseList && expenses.map((item) => (
            <TouchableOpacity
              key={item.id}
            ...
            >
              <View style={styles.rowBetween}>
                <Text style={{ fontWeight: "600", color: COLORS.TEXT_PRIMARY }}>{item.category}</Text>
                <Text style={{ color: COLORS.TEXT_SECONDARY, fontWeight: "700" }}>₹{item.amount}</Text>
              </View>
              {item.description ? (
                <Text style={{ color: COLORS.TEXT_SECONDARY, marginTop: 4 }} numberOfLines={1}>{item.description}</Text>
              ) : null}
              <View style={[styles.rowBetween, { marginTop: 8 }]}>
                <Text style={{ color: COLORS.TEXT_LIGHT, fontSize: 11 }}>{item.date} • {item.time}</Text>
                <TouchableOpacity onPress={(e) => { e.stopPropagation(); deleteExpenseItem(item.id); }}>
          ...
          {[
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                if (item === "Property Details") setShowPropertyDetailsModal(true);
                if (item === "Edit Building Structure") setShowStructureModal(true);
                if (item === "Transactions") Alert.alert("Transactions", "Transaction history coming soon.");
              }}
            >
            ...
            {!isEditing ? (
              ...
              <>
                <TextInput
                  ...
                  onChangeText={(text) => setEditableOwner({ ...editableOwner, name: text })}
                  style={styles.input}
                />
                <TextInput
                  value={editableOwner.email}
                  onChangeText={(text) => setEditableOwner({ ...editableOwner, email: text })}
                  style={styles.input}
                />
                <TextInput
                  value={editableOwner.phone}
                  onChangeText={(text) => setEditableOwner({ ...editableOwner, phone: text })}
                  style={styles.input}
                />
                <TouchableOpacity
                  style={styles.primaryButton}
                  onPress={() => { setIsEditing(false); Alert.alert("Profile Updated"); }}
                >
          ...
          <View style={styles.modalCard}>
            ...
            <View style={styles.detailBox}>
              ...
              <TouchableOpacity
                ...
                onPress={() => { deleteExpenseItem(selectedExpense?.id); setExpenseModalVisible(false); }}
              >
      ...
      <Modal visible={showPropertyDetailsModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, { height: '70%', borderTopLeftRadius: 35, borderTopRightRadius: 35 }]}>
            <View style={styles.modalHeader}>
              <View>
                <Text style={styles.modalTitle}>Property Details</Text>
                <Text style={{ fontSize: 12, color: COLORS.TEXT_LIGHT }}>Building Configuration & Capacity</Text>
              </View>
              <TouchableOpacity onPress={() => setShowPropertyDetailsModal(false)} style={styles.closeModalCircle}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={{ marginTop: 10 }}>
              <View style={styles.formSection}>
                <Text style={styles.formSectionTitle}>Capacity Information</Text>
                <FormRow label="Total Bed Capacity" value={property.totalBeds} icon="bed-outline" />
                <FormRow label="Currently Occupied" value={property.occupied} icon="people-outline" />
                <FormRow label="Available Beds" value={emptyBeds} icon="checkmark-circle-outline" highlight />
              </View>

              <View style={styles.formSection}>
                <Text style={styles.formSectionTitle}>Financial Config</Text>
                <FormRow label="Base Monthly Income" value={`₹${property.baseIncome}`} icon="cash-outline" />
              </View>

              <View style={styles.formSection}>
                <Text style={styles.formSectionTitle}>Operational Status</Text>
                <FormRow label="Building Status" value="Active" icon="business-outline" />
                <FormRow label="Utilities" value="Functional" icon="flash-outline" />
              </View>
            </ScrollView>

            <TouchableOpacity
              style={[styles.primaryButton, { marginTop: 20, borderRadius: 20, height: 55, justifyContent: 'center' }]}
              onPress={() => setShowPropertyDetailsModal(false)}
            >
              <Text style={[styles.primaryButtonText, { fontSize: 16 }]}>Back to Profile</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
      ...
      <Modal visible={showStructureModal} animationType="slide" transparent keyboardShouldPersistTaps="handled">
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, { height: '80%' }]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                    ...
                    {floor.rooms.map((room, rIdx) => (
                      <View key={rIdx} style={styles.roomItem}>
                        <View style={styles.rowBetween}>
                          ...
                          <Text style={styles.roomMeta}>{room.occupied}/{room.beds} Beds</Text>
                        </View>
              ...
              <TouchableOpacity
                style={[styles.addButton, { marginTop: 20, backgroundColor: COLORS.SUCCESS || '#4CAF50', flexDirection: 'row' }]}
                onPress={() => Alert.alert("Add Account", "Opening account setup form...")}
              >
...
const PieSlice = ({ color, percent, startAngle }) => {
  ...
  return (
    <View style={[styles.pieSliceBase, { transform: [{ rotate: `${rotation}deg` }] }]}>
      <View style={[
      ...
      {isLarge && (
        <View style={[styles.pieSliceInner, { backgroundColor: color, transform: [{ rotate: '180deg' }] }]} />
      )}
    </View>
  );
};
...
const FormRow = ({ label, value, icon, highlight }) => (
  <View style={styles.formRow}>
    ...
    <View style={{ flex: 1 }}>
      ...
      <Text style={[styles.formValue, highlight && { color: COLORS.SUCCESS, fontWeight: '800' }]}>{value}</Text>
    </View>
    {highlight && <Ionicons name="star" size={16} color="#FFD700" />}
  </View>
);

const LegendItem = ({ color, label, value, percentage, center }) => (
  <View style={{
    flexDirection: 'row',
    alignItems: 'center',
    marginBottom: 12,
    justifyContent: center ? 'center' : 'flex-start',
    width: center ? 'auto' : '48%'
  }}>
    <View style={{ width: 10, height: 10, borderRadius: 5, backgroundColor: color, marginRight: 8 }} />
    <View style={{ flex: 1 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        <Text numberOfLines={1} style={{ color: 'rgba(255,255,255,0.7)', fontSize: 10, fontWeight: '600', flex: 1 }}>{label}</Text>
        <Text style={{ color: 'rgba(255,255,255,0.5)', fontSize: 10 }}>{percentage}%</Text>
      </View>
      <Text style={{ color: COLORS.WHITE, fontSize: 14, fontWeight: '800' }}>₹{value}</Text>
    </View>
  </View>
);
...
const styles = StyleSheet.create({
  ...
  heroTitle: { fontSize: 26, fontWeight: "700", color: COLORS.WHITE, marginBottom: 20 },
  profileCard: { flexDirection: "row", alignItems: "center", backgroundColor: COLORS.WHITE, padding: 18, borderRadius: 20, elevation: 5 },
  avatar: { width: 65, height: 65, borderRadius: 32, backgroundColor: COLORS.PRIMARY_LIGHT, justifyContent: "center", alignItems: "center" },
  ...
  statCard: { width: "48%", backgroundColor: COLORS.WHITE, padding: 18, borderRadius: 18, elevation: 3 },
  statTitle: { fontSize: 12, color: COLORS.TEXT_SECONDARY },
  statValue: { fontSize: 20, fontWeight: "700", marginTop: 6, color: COLORS.TEXT_PRIMARY },
  profitCard: {
    marginTop: 10,
    borderRadius: 30,
    elevation: 8,
  ...
  profitValue: { color: COLORS.WHITE, fontSize: 26, fontWeight: "800", marginTop: 6 },
  propertyRow: { flexDirection: "row", justifyContent: "space-between", marginTop: 20 },
  miniStat: { width: "31%", backgroundColor: COLORS.WHITE, paddingVertical: 16, borderRadius: 16, alignItems: "center" },
  miniValue: { fontSize: 18, fontWeight: "800", color: COLORS.PRIMARY },
  miniLabel: { fontSize: 11, color: COLORS.TEXT_SECONDARY, marginTop: 4 },
  accountRow: { flexDirection: "row", justifyContent: "space-between", backgroundColor: COLORS.WHITE, padding: 18, borderRadius: 16, marginBottom: 10 },
  ...
  modalCard: { backgroundColor: COLORS.WHITE, padding: 25, borderTopLeftRadius: 25, borderTopRightRadius: 25 },
  ...
  detailValue: { fontSize: 16, fontWeight: "600", color: COLORS.TEXT_PRIMARY, marginTop: 4 },
  primaryButton: { backgroundColor: COLORS.PRIMARY, padding: 16, borderRadius: 14, alignItems: "center", marginTop: 15 },
  ...
  addButton: { backgroundColor: COLORS.PRIMARY, padding: 16, borderRadius: 14, alignItems: "center" },
  addButtonText: { color: COLORS.WHITE, fontWeight: "700", fontSize: 15 },
  expenseItem: { backgroundColor: COLORS.WHITE, padding: 14, borderRadius: 14, marginTop: 12 },
  expenseBox: { backgroundColor: COLORS.WHITE, padding: 18, borderRadius: 20 },
  sectionTitle: { fontSize: 16, fontWeight: "600", color: COLORS.TEXT_PRIMARY, marginBottom: 12, marginTop: 28 },
  ...
  detailBox: { backgroundColor: COLORS.BACKGROUND, padding: 20, borderRadius: 20, borderWidth: 1, borderColor: COLORS.BORDER },
  sheetDivider: { height: 1, backgroundColor: COLORS.DIVIDER, marginVertical: 15 },
  structureList: { marginTop: 10 },
  floorSection: { marginBottom: 20 },
  floorTitle: { fontSize: 18, fontWeight: '700', color: COLORS.PRIMARY, marginBottom: 10 },
  roomItem: {
